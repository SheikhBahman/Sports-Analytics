...
```
- As it can be seen RS, RA, W, OBP, SLG, BA, OOBP and OSLG seem to have a kind of linear relashinship with each other. We will see these feature variables are the most significants for our predictive model. lets also check the variation of some most important predictors:
...
```{r, fig.height=63, fig.width=30}

### plot_dist() function-----------------------------------------
    # plot histogram of specified predictor variable distribution

plot_dist = function(variable, variable_id, norm_dist = FALSE){
  par(mar = c(17, 9, 10, 5), mgp = c(6, 1, 0))
  # plot histogram of variable distribution
  hist(variable, freq = FALSE,
       xlab = "Variable Quantiles",
       main = variable_id, cex.main = 4,
       font = 1, family = "sans", cex.lab = 3.3,
       col = "slategrey", border = "white", cex.axis = 2.8)
  # only add true distribution for data belonging to a normal distribution
  if(isTRUE(norm_dist) == TRUE){
        # plot true normal distribution
        curve(dnorm(x, mean = mean(variable), sd = sd(variable)),
        col = "red", add = TRUE, lwd = 5)
        # plot mean of the normal distribution
        abline(v = mean(variable), lty = 2, lwd = 5, col = "paleturquoise3")
  }
}


par(mfrow = c(6, 3))
    plot_dist(Baseball_stats$G, "G - [Games]")
    plot_dist(Baseball_stats$R, "R - [Runs]", norm_dist = TRUE)
    plot_dist(Baseball_stats$HR, "HR - [Home runs]")
    plot_dist(Baseball_stats$RBI, "RBI - [Runs batted in]", norm_dist = TRUE)
    plot_dist(Baseball_stats$DBL, "DBL - [Doubles]", norm_dist = TRUE)
    plot_dist(Baseball_stats$TR, "TR - [Triples]")
    plot_dist(Baseball_stats$SB, "SB - [Stolen Bases]")
    plot_dist(Baseball_stats$CS, "CS - [Caught stealing]")
    plot_dist(Baseball_stats$SO, "SO - [Struck out]", norm_dist = TRUE)
    plot_dist(Baseball_stats$IBB, "IBB - [Intentional bases on balls]")
    plot_dist(Baseball_stats$SH, "SH - [Sacrifices]")
    plot_dist(Baseball_stats$SF, "SF - [Sacrifice flies]")
    plot_dist(Baseball_stats$GIDP, "GIDP - [Grounded into double play]", norm_dist = TRUE)
    plot_dist(Baseball_stats$AVG, "AVG - [Batting Average]", norm_dist = TRUE)
    plot_dist(Baseball_stats$OBP, "OBP - [On base percentage]", norm_dist = TRUE)
    plot_dist(Baseball_stats$SLG, "SLG - [Slugging percentage]", norm_dist = TRUE)
    plot_dist(Baseball_stats$EXP, "EXP - [Experience]")
    plot_dist(Baseball_stats$PAYR, "PAYR - [Plate appearances per year]", norm_dist = TRUE)
    
```

```{r, fig.height=63, fig.width=30}
# scatterplot() function-----------------------------------------------------------
### create function to generate scatterplot of a predictor against the response

scatterplot = function(variable, variable_id, title){
  par(mar = c(17, 9, 10, 5), mgp = c(6, 1, 0))
  # add scatterplot
  plot(SLG ~ variable, data = Baseball_stats,
       col = "slategrey", pch = 16, cex = 0.9,
       main = title, xlab = variable_id, 
       ylab = "SLG - [Slugging Percentage]",
       cex.main = 4, cex.lab = 3.3, cex.axis = 2.8)
}

par(mfrow = c(6, 3))
    scatterplot(Baseball_stats$G, "G - [Games]", "SLG vs. G")
    scatterplot(Baseball_stats$R, "R - [Runs]", "SLG vs. R")
    scatterplot(Baseball_stats$HR, "HR - [Home runs]", "SLG vs. HR")
    scatterplot(Baseball_stats$RBI, "RBI - [Runs batted in]", "SLG vs. RBI")
    scatterplot(Baseball_stats$DBL, "DBL - [Doubles]", "SLG vs. DBL")
    scatterplot(Baseball_stats$TR, "TR - [Triples]", "SLG vs. TR")
    scatterplot(Baseball_stats$SB, "SB - [Stolen Bases]", "SLG vs. SB")
    scatterplot(Baseball_stats$CS, "CS - [Caught stealing]", "SLG vs. CS")
    scatterplot(Baseball_stats$SO, "SO - [Struck out]", "SLG vs. G")
    scatterplot(Baseball_stats$IBB, "IBB - [Intentional bases on balls]", "SLG vs. SO")
    scatterplot(Baseball_stats$SH, "SH - [Sacrifices]", "SLG vs. SH")
    scatterplot(Baseball_stats$SF, "SF - [Sacrifice flies]", "SLG vs. SF")
    scatterplot(Baseball_stats$GIDP, "GIDP - [Grounded into double play]", "SLG vs. GIDP")
    scatterplot(Baseball_stats$AVG, "AVG - [Batting Average]", "SLG vs. AVG")
    scatterplot(Baseball_stats$OBP, "OBP - [On base percentage]", "SLG vs. OBP")
    scatterplot(Baseball_stats$SLG, "SLG - [Slugging percentage]", "SLG vs. SLG")
    scatterplot(Baseball_stats$EXP, "EXP - [Experience]", "SLG vs. EXP")
    scatterplot(Baseball_stats$PAYR, "PAYR - [Plate appearances per year]", "SLG vs. PAYR")
...
```
- So finally after trying different scenarios let's see the final model predictors and their distributions:

```{r fig.height=15, fig.width=15}
pairs(back_bic_data, col = "darkblue")
```
...
```{r}
#Model to predict batting average
...
In this project we used statistical analysis we have learned in STAT-420 course to perform a sports analytics on baseball data in order to mainly answer the following questions:

- Statistically predict how does a team make the playoffs?

we answered this question by just exploring the data. We found a better criteria to predict whether a team made to the playoffs or not. Based on our results team with 99 wins or higher would make to playoffs with a high chance. Also instead of using the original feature variables: RS and RA if you consider the difference between this two variables (RS-RA) we can find a linear relationship with tem wins (see below charts).
...
We developed a classifier to predict whether a team made to the playoffs or not. After trying different predictors and different models we come up with a logistic model with the following predictors: Year + OBP + SLG + BA + RunDiff + OBP:SLG + SLG:BA + SLG:RunDiff

Our final best classifier has sensitivity, specificity and misclassification rate as follow, you may see the other models we built in the building the classifier section:
...
- What evidences make a team more winner? How does a team score more runs?
- Predict a player's batting average, to assess the player quality for the post.

To predict team wins, runs scored, run allowed, and player's batting average we fitted four different regression models:
The models are fitted using AIC, BIC and trial and error. After fitting each models we checked the regression assumptions and R squared of each models separately, the final models are briefly as follow:
